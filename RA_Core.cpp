...
EmulatorID g_EmulatorID = UnknownEmulator;	//	Uniquely identifies the emulator
ConsoleID g_ConsoleID = UnknownConsoleID;	//	Currently active Console ID
...
API BOOL CCONV _RA_InitI( HWND hMainHWND, /*enum EmulatorID*/int nEmulatorID, const char* sClientVer )
{
	...
	do
	{
		...
		{
			nMBReply = MessageBox( nullptr,
								   _T( "Cannot load RA_Keys.dll!\n" )
								   _T( "Download fresh RetroAchievement keys file from RetroAchievements.org?" ),
								   _T( "Warning" ), 
								   MB_YESNO );
...
bool _InstallKeys()
{
	...
	g_hRAKeysDLL = LoadLibrary( L"RA_Keys.dll" );
