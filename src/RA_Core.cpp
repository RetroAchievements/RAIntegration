...
{
    g_MemManager.AddMemoryBank(static_cast<size_t>(nBankID), (_RAMByteReadFn*)pReader, (_RAMByteWriteFn*)pWriter,
                               static_cast<size_t>(nBankSize));
...
static bool RA_OfferNewRAUpdate(const char* sNewVer)
{
    ...
    while (sClientVersion.back() == '0' && sClientVersion.at(sClientVersion.length() - 2) == '.')
        sClientVersion.resize(sClientVersion.length() - 2);

    std::wostringstream oss;
    oss << L"A new version of " << ra::Widen(g_sClientName) << L" is available for download at "
        << ra::Widen(_RA_HostName()) << L".\n\n"
    ...
    if (vmMessageBox.ShowModal() == ra::ui::DialogResult::Yes)
    {
        ...
        ShellExecute(nullptr, TEXT("open"), NativeStr(oss2.str()).c_str(), nullptr, nullptr, SW_SHOWNORMAL);
            ...
            switch (pObj->GetRequestType())
            {
                ...
                case RequestSubmitAwardAchievement:
                {
                    ...
                    else
                    {
                        ...
                        RA_LOG("RequestSubmitAwardAchievement responded, but cannot find achievement with ID %u",
                               nAchID);
                    }
                }
                break;
        ...
        if (doc.HasMember("LatestVersion"))
        {
            ...
            else
            {
                ...
                std::wstring sMessage =
                    L"You already have the latest version of " + ra::Widen(g_sClientName) + L": " + ra::Widen(sReply);
...
API void CCONV _RA_InvokeDialog(LPARAM nID)
{
    switch (nID)
    {
        ...
        case IDM_RA_FILES_LOGOUT:
        {
            ...
            {
                ra::ui::viewmodels::MessageBoxViewModel::ShowErrorMessage(L"Logout failed",
            ...
            else
            {
                ...
                if (pGameContext.GameId() != 0)
                {
                    ...
                    vmMessageBox.SetMessage(
                        L"Enabling Hardcore mode will reset the emulator. You will lose any progress that has not been "
                        L"saved through the game.");
                    vmMessageBox.SetIcon(ra::ui::viewmodels::MessageBoxViewModel::Icon::Warning);
                    vmMessageBox.SetButtons(ra::ui::viewmodels::MessageBoxViewModel::Buttons::YesNo);
...
{
    // To use time_t we need local time
    std::tm tm{};
    std::ostringstream oss;
    Expects(localtime_s(&tm, &nTime) == 0);
    oss << std::put_time(&tm, "%c"); // full timestamp
    return oss.str();
}
...
_Success_(return == 0) _NODISCARD static auto CALLBACK
    BrowseCallbackProc(_In_ HWND hwnd, _In_ UINT uMsg, _In_ _UNUSED LPARAM lParam,
                       _In_ LPARAM lpData) noexcept // it might
{
