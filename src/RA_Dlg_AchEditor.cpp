...
INT_PTR CALLBACK AchProgressProc(HWND hDlg, UINT nMsg, WPARAM wParam, _UNUSED LPARAM lParam)
{
    switch (nMsg)
    {
        case WM_INITDIALOG:
        ...
        case WM_COMMAND:
            switch (LOWORD(wParam))
            {
                ...
                case IDC_RA_ADDCOND:
                {
                    ...
                    //	Select last item
                    HWND hList = GetDlgItem(hDlg, IDC_RA_LBX_CONDITIONS);
                    // to_unsigned on any negative integer will make it the max size of it's unsigned counterpart
        ...
        case WM_NOTIFY:
        {
            switch ((((LPNMHDR)lParam)->code))
            {
                case NM_CLICK:
                {
                    ...
                    _UNUSED HWND hList = GetDlgItem(m_hAchievementEditorDlg, IDC_RA_LBX_CONDITIONS);
...
void Dlg_AchievementEditor::LoadAchievement(Achievement* pCheevo, _UNUSED BOOL bAttemptKeepSelected)
{
    char buffer[1024];
