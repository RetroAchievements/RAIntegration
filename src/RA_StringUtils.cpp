...
std::string Narrow(const wchar_t* wstr)
{
    ...
    std::string str(ra::to_unsigned(needed), {}); // allocate required space (including terminator)
    ::WideCharToMultiByte(CP_UTF8, 0, wstr, len + 1, str.data(), gsl::narrow_cast<int>(str.capacity()), nullptr,
                          nullptr);
...
std::wstring Widen(const char* str)
{
    ...
    ::MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, str, len + 1, wstr.data(),
                          gsl::narrow_cast<int>(wstr.capacity()));
