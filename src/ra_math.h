...
_NODISCARD inline auto ftol(_In_ FloatingPoint fp) noexcept { return std::lround(fp); }

template<typename FloatingPoint, class = std::enable_if_t<std::is_floating_point_v<FloatingPoint>>>
_NODISCARD inline auto ftoll(_In_ FloatingPoint fp) noexcept { return std::llround(fp); }

template<typename FloatingPoint, class = std::enable_if_t<std::is_floating_point_v<FloatingPoint>>>
_NODISCARD inline auto ftoul(_In_ FloatingPoint fp) noexcept { return to_unsigned(std::lround(fp)); }

template<typename FloatingPoint, class = std::enable_if_t<std::is_floating_point_v<FloatingPoint>>>
_NODISCARD inline auto ftoull(_In_ FloatingPoint fp) noexcept { return to_unsigned(std::llround(fp)); }

template<typename Arithmetic, class = std::enable_if_t<std::is_arithmetic_v<Arithmetic>>>
_NODISCARD inline auto sqr(_In_ Arithmetic a) noexcept { return std::pow(a, 2); }
