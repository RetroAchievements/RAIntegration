...
void SearchResults::Initialize(ra::ByteAddress nAddress, unsigned int nBytes, MemSize nSize)
{
    ...
    if (gsl::narrow_cast<std::size_t>(nBytes + nAddress) > g_MemManager.TotalBankSize())
    ...
    while (nBytes > 0)
    {
        ...
        g_MemManager.ActiveBankRAMRead(block.GetBytes(), block.GetAddress(),
                                       gsl::narrow_cast<std::size_t>(nBlockSize + nPadding));
