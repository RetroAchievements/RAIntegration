...
WindowsFileSystem::WindowsFileSystem() noexcept
{
    ...
    GetModuleFileNameW(nullptr, sBuffer.data(), MAX_PATH);
    PathRemoveFileSpecW(sBuffer.data());
    m_sBaseDirectory = sBuffer.data();
...
{
    return (CreateDirectoryW(sDirectory.c_str(), nullptr) != 0);
}
...
int64_t WindowsFileSystem::GetFileSize(const std::wstring& sPath) const
{
    ...
    const LARGE_INTEGER nSize{ fadFile.nFileSizeLow, gsl::narrow_cast<LONG>(fadFile.nFileSizeHigh) };
