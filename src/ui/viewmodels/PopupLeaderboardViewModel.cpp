...
void PopupMessageViewModel::UpdateRenderImage(double fElapsed)
{
    ...
    {
        // fading in
        auto fPercentage = INOUT_TIME - m_fAnimationProgress;
        auto nY = m_nInitialY + (m_nInitialY - m_nTargetY) * (fPercentage * fPercentage);
    ...
    {
        // fading out
        auto fPercentage = TOTAL_ANIMATION_TIME - m_fAnimationProgress;
        auto nY = m_nInitialY + (m_nInitialY - m_nTargetY) * (fPercentage * fPercentage);
        SetRenderLocationY(static_cast<int>(nY));
    }
    else
