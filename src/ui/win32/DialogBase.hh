...
public:
	...
	/// <summary>Creates the dialog window (but does not show it).</summary>
	...
	/*_NODISCARD */HWND CreateDialogWindow(_In_z_ const LPCTSTR sResourceId,
										   _In_   IDialogPresenter* const pDialogPresenter);

	/// <summary>Gets the <see cref="HWND" /> for the dialog.</summary>
	_NODISCARD HWND GetHWND() const { return m_hWnd; }

	/// <summary>Shows the dialog window.</summary>
	/// <returns>
	///   <c>true</c> if the window was shown, <c>false</c> if CreateDialogWindow has not been called.
	/// </returns>
...
protected:
	...
	/// <summary>Callback for processing <c>WINAPI</c> messages - do not call directly!</summary>
	_NODISCARD virtual INT_PTR CALLBACK DialogProc(HWND, UINT, WPARAM, LPARAM);

	/// <summary>Called when the window is created, but before it is shown.</summary>
	/// <returns>
	///   Return <c>TRUE</c> if passing the keyboard focus to a default control, otherwise return <c>FALSE</c>.
	/// </returns>
	virtual void OnInitDialog() {}

	/// <summary>Called when the window is moved.</summary>
	virtual void OnDestroy();

	/// <summary>Called when a button is clicked.</summary>
...
private:
	friend static INT_PTR CALLBACK StaticDialogProc(_In_ HWND hDlg, _In_ UINT uMsg, _In_ WPARAM wParam,
													_In_ LPARAM lParam);
