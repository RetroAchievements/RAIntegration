...
public:
    ...
    void Start() noexcept override { m_bActive = true; }

    void Cancel() noexcept override { m_bActive = false; }
...
TEST_CLASS(RA_Leaderboard_Tests){void AssertValue(const char* sSerialized, unsigned int nExpected){
...
TEST_METHOD(TestSimpleLeaderboard)
{
    ...
    memory.front() = 3; // submit value, but not active
    lb.Test();
    Assert::IsFalse(lb.IsActive());
    Assert::IsFalse(lb.IsScoreSubmitted());

    memory.front() = 2; // cancel value, but not active
    lb.Test();
    Assert::IsFalse(lb.IsActive());
    Assert::IsFalse(lb.IsScoreSubmitted());

    memory.front() = 1; // start value
    lb.Test();
    Assert::IsTrue(lb.IsActive());
    Assert::IsFalse(lb.IsScoreSubmitted());

    memory.front() = 2; // cancel value
    lb.Test();
    Assert::IsFalse(lb.IsActive());
    Assert::IsFalse(lb.IsScoreSubmitted());
...
TEST_METHOD(TestStartAndCancelSameFrame)
{
    ...
    memory.front() = 0x01; // disables start; no effect this frame, but next frame can restart
    lb.Test();
    Assert::IsFalse(lb.IsActive());
    Assert::IsFalse(lb.IsScoreSubmitted());

    memory.front() = 0x00; // enables start
    lb.Test();
    Assert::IsTrue(lb.IsActive());
    Assert::IsFalse(lb.IsScoreSubmitted());
}
...
TEST_METHOD(TestStartAndSubmitSameFrame)
{
    ...
    memory.front() = 1; // disable start
    lb.Test();
    Assert::IsFalse(lb.IsActive());

    memory.front() = 0; // reenable start, leaderboard should reactivate
    lb.Test();
    Assert::IsTrue(lb.IsActive());
}
...
TEST_METHOD(TestStartOrCondition)
{
    ...
    memory.front() = 1; // first part of start condition is true
    lb.Test();
    Assert::IsTrue(lb.IsActive());
}
